/*
  ...
  NOTE: names used in the random creation process have been taken from the moby project https://github.com/moby/moby
...
func RandomName(number int) string {
	sep := separator[rand.Intn(len(separator))]
	result := adjective[rand.Intn(len(adjective))] + sep + name[rand.Intn(len(name))]
	if number > 0 {
		result = fmt.Sprintf("%s%s%d", result, sep, rand.Intn(number))
	}
	return result
}
