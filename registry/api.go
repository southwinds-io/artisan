...
func (r *Api) GetAllRepositoryInfo(user, pwd string, showWarnings bool) ([]Repository, error, int, bool) {
	useTls := true
	repo, err, code := r.getAllRepositoryInfo(user, pwd, useTls)
	if err != nil {
		// try without tls
		var err2 error
		...
		if err2 == nil {
			...
			if err2 != nil {
				return nil, fmt.Errorf("cannot retrieve remote registry information: %s", err2), http.StatusInternalServerError, useTls
			}
		}
	}
	return repo, nil, code, useTls
}
...
func (r *Api) Download(group, name, filename, user, pwd string, isSeal, https bool) (string, error, int) {
	...
	if res != nil {
		...
		if res.StatusCode > 299 {
			if err != nil {
				return "", fmt.Errorf("failed to download file: %s", err), status
			} else {
				return "", fmt.Errorf("failed to download file: %s", res.Status), status
			}
		}
	}
	if err != nil {
...
func (r *Api) packageWithTagURI(group, name, tag string, https bool) string {
	// note: do not escape group here as r.packageURI does, otherwise it will be double encoded
